...
#define THRESHOLD1 250 //In order to determine the state of the hand (opened/closed)
#define THRESHOLD2 300 //In order to determine the state of the hand (opened/closed)
...
//Servo servoWrist;       //Define wrist servo
...
void opendigit(int digit){
   ...
   servoThumb.write(0);
   /* //Add to control fingers separately
...
void loop() {// Nothing to do here, all is done in the interrupt function
  ...
  if(value>THRESHOLD1||value>THRESHOLD2) //If the value of the sample is greater than THRESHOLD means that the hand has been closed
  {closehand();}  
  ...
  int value1 = analogRead(EMGPIN0); //Sampling analog signal
